...
<html><head><title>Signup - CoogsNation</title></head><body>
<h1>Signup</h1>
...
app.get("/post",(req,res)=>res.send(`<!DOCTYPE html>
<html><head><title>Create Post - CoogsNation</title></head><body>
<h1>Create Post</h1>
<form id="f">
<input name="title" placeholder="Post Title"><br><br>
<textarea name="content" placeholder="Post Content" rows="4" cols="50"></textarea><br><br>
...
f.onsubmit=async e=>{
  ...
  const r=await fetch('/api/moderate-post',{
    method:'POST',
...
app.get("/ask",(req,res)=>res.send(`<!DOCTYPE html>
<html><head><title>Ask AI - CoogsNation</title></head><body>
<h1>Ask AI</h1>
<form id="f">
<textarea name="q" placeholder="Ask a question..." rows="3" cols="50"></textarea><br><br>
...
f.onsubmit=async e=>{
  ...
  const r=await fetch('/api/ask',{
    method:'POST',
    headers:{'Content-Type':'application/json'},
    body:JSON.stringify({question:f.q.value})
  });
