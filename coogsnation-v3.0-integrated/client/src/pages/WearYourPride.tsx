...
export default function WearYourPride() {
  const { user, isAuthenticated } = useAuth();
  const [selectedSort, setSelectedSort] = useState<string>("name");
  const { toast } = useToast();

  // Fetch Wear Your Pride products from Printful
  const { data: podResponse, isLoading, error } = useQuery<PODResponse>({
    queryKey: ["/api/store/wear-your-pride"],
  ...
  const addToCartMutation = useMutation({
    mutationFn: ({ productId, quantity }: { productId: string; quantity: number }) => {
      // For POD products, we may need to handle differently since they use string IDs
      // For now, let's convert to number if needed
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h1 className="text-4xl font-bold text-uh-black mb-2" data-testid="heading-wear-your-pride">
                Wear Your Pride
              </h1>
              <p className="text-lg text-gray-600 mb-4">
                Premium University of Houston apparel by Printful
              </p>
              <div className="flex items-center space-x-4 text-sm text-gray-500">
                <span className="flex items-center">
                  <i className="fas fa-tshirt mr-2 text-uh-red"></i>
                  Professional Polos & Jackets
                </span>
                <span className="flex items-center">
                  <i className="fas fa-hat-cowboy mr-2 text-uh-red"></i>
                  Embroidered Hats
                </span>
                <span className="flex items-center">
                  <i className="fas fa-shipping-fast mr-2 text-uh-red"></i>
                  Print on Demand
                </span>
              </div>
            </div>
          ...
          <Card className="mb-8">
            <CardContent className="p-6 text-center">
              ...
              <p className="text-gray-600 mb-4">
                We're having trouble connecting to our apparel provider. Please try again later.
              </p>
              <Button 
                onClick={() => queryClient.invalidateQueries({ queryKey: ["/api/store/wear-your-pride"] })}
                variant="outline"
            ...
            {sortedProducts.map((product) => (
              <Card key={product.id} className="hover:shadow-lg transition-shadow duration-200" data-testid={`card-product-${product.id}`}>
                <div className="relative">
                  <img
                    src={product.image || "https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?w=400&h=300&fit=crop"}
                    alt={product.name}
                  ...
                  <Badge 
                    variant="secondary"
                    className="absolute top-2 left-2 bg-blue-100 text-blue-700"
                  >
                    <i className="fas fa-print mr-1 text-xs"></i>
                    Printful
                  </Badge>
                </div>
          ...
          <Card>
            <CardContent className="p-8 text-center">
              ...
              <p className="text-gray-600 mb-6">
                We're currently updating our Wear Your Pride collection. Check back soon for new arrivals!
              </p>
              <Button 
                onClick={() => queryClient.invalidateQueries({ queryKey: ["/api/store/wear-your-pride"] })}
                variant="outline"
        ...
        <div className="mt-12 bg-white p-6 rounded-lg shadow-sm">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
            <div>
              <i className="fas fa-award text-3xl text-uh-red mb-3"></i>
              <h3 className="font-bold text-uh-black mb-2">Premium Quality</h3>
              <p className="text-gray-600">Professional-grade apparel from Printful</p>
            </div>
            <div>
              <i className="fas fa-palette text-3xl text-uh-red mb-3"></i>
              <h3 className="font-bold text-uh-black mb-2">Custom Designs</h3>
              <p className="text-gray-600">University of Houston branded merchandise</p>
            </div>
            <div>
              <i className="fas fa-shipping-fast text-3xl text-uh-red mb-3"></i>
              <h3 className="font-bold text-uh-black mb-2">Print on Demand</h3>
              <p className="text-gray-600">Fresh, made-to-order apparel</p>
            </div>
          </div>
        </div>
