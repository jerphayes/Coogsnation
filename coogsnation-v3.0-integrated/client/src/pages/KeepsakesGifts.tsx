...
export default function KeepsakesGifts() {
  const { user, isAuthenticated } = useAuth();
  const [selectedSort, setSelectedSort] = useState<string>("name");
  const { toast } = useToast();

  // Fetch Keepsakes & Gifts products from Trendsi
  const { data: podResponse, isLoading, error } = useQuery<PODResponse>({
    queryKey: ["/api/store/keepsakes-gifts"],
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h1 className="text-4xl font-bold text-uh-black mb-2" data-testid="heading-keepsakes-gifts">
                Keepsakes & Gifts
              </h1>
              <p className="text-lg text-gray-600 mb-4">
                Premium jewelry and accessories by Trendsi
              </p>
              <div className="flex items-center space-x-4 text-sm text-gray-500">
                <span className="flex items-center">
                  <i className="fas fa-ring mr-2 text-uh-red"></i>
                  Fine Jewelry
                </span>
                <span className="flex items-center">
                  <i className="fas fa-gem mr-2 text-uh-red"></i>
                  Premium Accessories
                </span>
                <span className="flex items-center">
                  <i className="fas fa-gift mr-2 text-uh-red"></i>
                  Perfect Gifts
                </span>
              </div>
            </div>
          ...
          <Card className="mb-8">
            <CardContent className="p-6 text-center">
              ...
              <p className="text-gray-600 mb-4">
                We're having trouble connecting to our jewelry provider. Please try again later.
              </p>
              <Button 
                onClick={() => queryClient.invalidateQueries({ queryKey: ["/api/store/keepsakes-gifts"] })}
                variant="outline"
            ...
            {sortedProducts.map((product) => (
              <Card key={product.id} className="hover:shadow-lg transition-shadow duration-200" data-testid={`card-product-${product.id}`}>
                <div className="relative">
                  <img
                    src={product.image || "https://images.unsplash.com/photo-1515562141207-7a88fb7ce338?w=400&h=300&fit=crop"}
                    alt={product.name}
                  ...
                  <Badge 
                    variant="secondary"
                    className="absolute top-2 left-2 bg-purple-100 text-purple-700"
                  >
        ...
        {!isLoading && !error && sortedProducts.length === 0 && (
          <Card>
            <CardContent className="p-8 text-center">
              <div className="mb-4">
                <i className="fas fa-gem text-4xl text-gray-400"></i>
              </div>
              <h3 className="text-xl font-bold mb-2">No Products Available</h3>
              <p className="text-gray-600 mb-6">
                We're currently updating our Keepsakes & Gifts collection. Check back soon for new arrivals!
              </p>
              <Button 
                onClick={() => queryClient.invalidateQueries({ queryKey: ["/api/store/keepsakes-gifts"] })}
                variant="outline"
        ...
        <div className="mt-12 bg-white p-6 rounded-lg shadow-sm">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
            <div>
              <i className="fas fa-certificate text-3xl text-uh-red mb-3"></i>
              <h3 className="font-bold text-uh-black mb-2">Premium Quality</h3>
              <p className="text-gray-600">Fine materials and craftsmanship from Trendsi</p>
            </div>
            <div>
              <i className="fas fa-heart text-3xl text-uh-red mb-3"></i>
              <h3 className="font-bold text-uh-black mb-2">Perfect Gifts</h3>
              <p className="text-gray-600">Meaningful keepsakes for special occasions</p>
            </div>
            <div>
              <i className="fas fa-university text-3xl text-uh-red mb-3"></i>
              <h3 className="font-bold text-uh-black mb-2">UH Pride</h3>
              <p className="text-gray-600">Elegant University of Houston themed pieces</p>
            </div>
          </div>
        </div>
