...
export class ShopifyCommerceProvider implements CommerceProvider {
  ...
  capabilities() {
    ...
    return {
      ...
      inquiries: false,
      /* Not implemented in v3.0, and declared false rather than omitted.
       * An absent capability reads as "unknown" to any caller; an explicit
       * false is a promise the application can rely on. Checkout happens in
       * the Shopify storefront until an embedded variant cart is built. */
      cartRead: false,
      cartMutation: false,
      checkoutUrl: false,
      note: configured
