...
function phaseFromStatus(status: Recordish): GamePhase {
  ...
  if (status?.type?.completed === true || state.includes("post")) {
    return "final";
  }
...
function endpoints(game: GameRef): string[] {
  const league =
  ...
  // A Saturday-night US game can already be Sunday in UTC.
  const previousDate = new Date(
    scheduled.getTime() - 86_400_000,
  );

  return [...new Set(
    [scheduled, previousDate].map(
      (date) =>
        `https://site.api.espn.com/apis/site/v2/sports/${sport}/${league}/scoreboard?dates=${dateParam(date)}`,
    ),
  )];
}
...
class EspnScoreboardAdapter implements SportsSourceAdapter {
  ...
  async fetchGame(
    ...
    for (const url of endpoints(game)) {
...
export function createEspnFootballScoreboardAdapter(
  fetchImpl: typeof fetch = fetch,
) {
  return new EspnScoreboardAdapter(
    "espn-football-public",
    "ESPN public college football scoreboard",
    "football",
    fetchImpl,
  );
}

export function createEspnBasketballScoreboardAdapter(
  fetchImpl: typeof fetch = fetch,
) {
  return new EspnScoreboardAdapter(
    "espn-basketball-public",
    "ESPN public men's college basketball scoreboard",
    "basketball",
    fetchImpl,
  );
}
